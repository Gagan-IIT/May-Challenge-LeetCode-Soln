...
class Solution {
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        return floodFillUtil(image, sr, sc, newColor, oldColor);
    }
    
    public int[][] floodFillUtil(int[][] image, int sr, int sc, int newColor, int oldColor) {
        int R = image.length;
        int C = image[0].length;
        if(image[sr][sc]!=oldColor) return image;
        image[sr][sc]=newColor;
        if(sr+1<R) image = floodFillUtil(image, sr+1, sc, newColor, oldColor);
        if(sr-1>=0) image = floodFillUtil(image, sr-1, sc, newColor, oldColor);
        if(sc+1<C) image = floodFillUtil(image, sr, sc+1, newColor, oldColor);
        if(sc-1>=0) image = floodFillUtil(image, sr, sc-1, newColor, oldColor);
        return image;
    }
}
